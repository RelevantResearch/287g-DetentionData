...
# -------------------------
# Read filename from txt file
# -------------------------
def get_filename_from_txt(txt_path: str) -> str:
    with open(txt_path, 'r') as f:
        return f.read().strip()
...
# -------------------------
def generate_pivot_report(filepath: str, output_dir: str) -> None:
    base_dir = os.getcwd()
    parent_dir = os.path.dirname(base_dir)
    df = pd.read_excel(filepath)
    ...
    status_pivot = pd.pivot_table(
        df, index="STATE", columns="STATUS", aggfunc="size", fill_value=0
    ).reset_index()
    status_pivot = add_total_row(status_pivot)
    ...
    # --- Pivot 3: Agencies with more than one agreement
    multiple_agreements = df.groupby("Agency Validation").size().reset_index(name="Agreement Count")
    multiple_agreements = multiple_agreements[multiple_agreements["Agreement Count"] > 1]
    multiple_agreements = add_total_row(multiple_agreements)
    ...
    # --- Ensure output directory exists
    pivot_dir = os.path.join(parent_dir, "pivotPendingAgencies")
    os.makedirs(pivot_dir, exist_ok=True)
...
# Runner
# -------------------------
def run_pending_agencies_pivot_report() -> None:
    base_dir = os.getcwd()
    parent_dir = os.path.dirname(base_dir)
    txt_file = os.path.join(base_dir, "last-pending.txt")
    total_pending_dir = os.path.join("Normalized_Total_pendingAgencies")
...
if __name__ == "__main__":
    print("Starting pivot table generation...\n")
    try:
